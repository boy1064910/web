...
Ding.FileUploader = function(config){
	...
	this.showChecker = Ding.isEmpty(config.showChecker) ? false : config.showChecker;
	...
	this.initCallback = Ding.isEmpty(config.initCallback) ? null : config.initCallback;//上传控制器初始化完成回调函数
				...
				for(var f in files){
					...
					if(_this.needPreview){
						if(files[f] || !/image\//.test(files[f].type)){//属于图片文件，进行预览展示
							...
							if(_this.showChecker){
								var checker = $('<input type="checkbox" class="img-preview-div-checker" />');
								jpreivewImgDiv.append(checker);
								jpreivewImgDiv.on("click",fileUploaderCheckEvent);
							}
						}
	...
	this.uploader.init();
	if(!Ding.isEmpty(_this.initCallback)){
		_this.initCallback();
	}
};
...
Ding.FileUploader.prototype.empty = function(){
	this.jcontainer.empty();
}

Ding.FileUploader.prototype.append = function(data){
	var jpreviewDiv = $('<div class="preview-div"></div>');
	var jpreivewImgDiv = $('<div class="img-preview-div"></div>');
	jpreivewImgDiv.css('background-image','url('+data.url+')');
	if(this.showChecker){
		var checker = $('<input type="checkbox" class="img-preview-div-checker" data-id="'+array[i].dataId+'" />');
		jpreivewImgDiv.append(checker);
		jpreivewImgDiv.on("click",fileUploaderCheckEvent);
	}
	jpreviewDiv.append(jpreivewImgDiv);
	this.jcontainer.append(jpreviewDiv);
}

Ding.FileUploader.prototype.appendArray = function(array){
	for(var i=0;i<array.length;i++){
		...
		jpreivewImgDiv.css('background-image','url('+array[i].url+')');
		jpreviewDiv.append(jpreivewImgDiv);
		this.jcontainer.append(jpreviewDiv);
	}
}

Ding.FileUploader.prototype.getCheckedList = function(){
	var checkerList = this.jcontainer.find('input[type="checkbox"][class="img-preview-div-checker"]');
	var checkedList = [];
	for(var i=0;i<checkerList.length;i++){
		if(checkerList[i].checked){
			checkedList.push(checkerList[i]);
		}
	}
	return checkedList;
}

Ding.FileUploader.prototype.setCheckerDataId = function(oid,dataId){
	var previewDiv = $("#progress"+oid).parent();
	previewDiv.find('input[type="checkbox"][class="img-preview-div-checker"]').attr('data-id',dataId);
}

function fileUploaderCheckEvent(){
	var checkbox = $(this).find(".img-preview-div-checker");
